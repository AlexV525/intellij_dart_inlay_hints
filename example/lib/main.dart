...
void main() {
  ...
  var stringMap = {'key': 'value'};          // Should show: stringMap: Map<String, String>
  ...
  final (fb1, fb2, fb3) = foo.toRecord();    // Should show: fb1: String, fb2: String, fb3: String?
  var (a, b) = (1, 'hello');                 // Should show: a: int, b: String
  final (x, y, z) = (1.0, true, null);       // Should show: x: double, y: bool, z: Null
  
  // For-each loops
  for (var e in <int>[1,2,3]) {              // Should show: e: int
    print(e);
  }
  
  for (var s in ['a', 'b', 'c']) {           // Should show: s: String
    print(s);
  }
  
  for (var item in list) {                   // Should show: item: int (from homogeneous inference)
    print(item);
  }
  
  for (var char in 'hello'.characters) {     // Should show: char: String
    print(char);
  }
  
  // Iterable.generate for ranges (Dart way)
  for (var i in Iterable.generate(10, (index) => index)) {  // Should show: i: int
    print(i);
  }

  // List.generate
  for (var num in List.generate(5, (i) => i * 2)) {        // Should show: num: int
    print(num);
  }
